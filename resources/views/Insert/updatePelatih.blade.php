...
<head>
    ...
    <title>Update Pelatih - Dinas Pariwisata Pemuda dan Olahraga</title>
...
<body>
    <nav class="navbar">
        <div class="nav-container">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item">
                    <a href="{{ route('data.pelatih') }}" class="nav-link active">Pelatih</a>
                </li>
                <li class="nav-item">
        ...
        <div class="page-container mt-32 sm:mt-40">
            <div class="page-header-card">
                <div class="flex justify-between items-center">
                    <h1 class="page-title">Update Data Pelatih</h1>
                    <a href="{{ route('data.pelatih') }}" class="btn-tambah" style="background: linear-gradient(135deg, #6b7280, #4b5563); text-decoration: none;">
                        <span>←</span> Kembali
                    </a>
                </div>
            </div>
            ...
            <div class="table-container-card">
                <div class="table-header" style="border-bottom: none; margin-bottom: 0; padding-bottom: 0;">
                    <h2 class="table-title">Form Data Pelatih</h2>
                </div>
                
                <form id="pelatihForm" action="{{ route('simpan.update.pelatih') }}" method="POST" enctype="multipart/form-data" style="margin-top: 30px;">
                    @csrf
                    
                    <input type="hidden" name="id_pelatih" value="{{ $dataPelatih->id }}">
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 25px; position: relative;">
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            ...
                            <select id="jenis_kelamin" 
                                ...
                                <option value="Laki-laki" {{ $dataPelatih->jenis_kelamin == 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                                <option value="Perempuan" {{ $dataPelatih->jenis_kelamin == 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                            </select>
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            ...
                            <select id="cabor" 
                                ...
                                @foreach ($dataCabor as $cabor)
                                    <option value="{{ $cabor->id }}" {{ $dataPelatih->id_cabor == $cabor->id ? 'selected' : '' }}>{{ $cabor->nama_cabor }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            ...
                            <input type="date" 
                                   ...
                                   name="tanggal_lahir" 
                                   required
                                   value="{{ $dataPelatih->tanggal_lahir }}"
                                   style="padding: 12px 16px; border: 2px solid #e0e0e0; border-radius: 12px; font-size: 14px; transition: all 0.3s ease; background: rgba(255, 255, 255, 0.8);"
                                   onfocus="this.style.borderColor='#667eea'; this.style.boxShadow='0 0 0 3px rgba(102, 126, 234, 0.1)'"
                                   onblur="this.style.borderColor='#e0e0e0'; this.style.boxShadow='none'">
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            ...
                            <input type="date" 
                                   ...
                                   name="tanggal_gabung" 
                                   required
                                      value="{{ $dataPelatih->tanggal_gabung }}"
                                   style="padding: 12px 16px; border: 2px solid #e0e0e0; border-radius: 12px; font-size: 14px; transition: all 0.3s ease; background: rgba(255, 255, 255, 0.8);"
                                   onfocus="this.style.borderColor='#667eea'; this.style.boxShadow='0 0 0 3px rgba(102, 126, 234, 0.1)'"
                                   onblur="this.style.borderColor='#e0e0e0'; this.style.boxShadow='none'">
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            ...
                            <select id="status" 
                                ...
                                <option value="Aktif" {{ $dataPelatih->status == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="Tidak Aktif" {{ $dataPelatih->status == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            ...
                            <input type="file" id="foto" name="foto" accept="image/*" style="padding: 8px;">
                                @if(!empty($dataPelatih->foto))
                                    <p style="font-size:12px;color:#666;margin-top:8px;">Foto saat ini: <a href="{{ asset('storage/' . $dataPelatih->foto) }}" target="_blank">Lihat</a></p>
                                @endif
                        </div>

                        <div style="grid-column: 1 / -1; padding:16px; border:1px solid #e5e7eb; border-radius:12px; background:#fafafa;">
                            <h3 style="font-weight:600; margin-bottom:10px;">📄 Dokumen Pelatih (PDF/Gambar)</h3>
                            <div style="display:flex; gap:10px; align-items:flex-start; flex-wrap:wrap;">
                                @csrf
                                <input type="hidden" name="pelatih_id" value="{{ $dataPelatih->id }}" form="uploadPelatihForm">
                                <input type="text" name="kategori" placeholder="Kategori (mis: Lisensi, Piagam, Sertifikat)" style="flex:1; min-width:240px; padding:10px; border:1px solid #e5e7eb; border-radius:8px;" form="uploadPelatihForm">
                                <input type="file" name="files[]" multiple accept=".pdf,image/*" style="padding:8px;" form="uploadPelatihForm">
                                <button type="submit" class="btn-tambah" style="padding:10px 16px;" form="uploadPelatihForm">Unggah</button>
                            </div>
                            <!-- hidden upload form moved below to avoid nested forms -->
                            @php($docs = $dataPelatih->documents ?? collect())
                            @if($docs->count())
                                <div style="margin-top:12px; display:grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap:10px;">
                                    @foreach($docs as $doc)
                                        <div style="border:1px solid #e5e7eb; border-radius:10px; padding:10px; background:#fff; display:flex; justify-content:space-between; align-items:center; gap:10px;">
                                            ...
                                            <form action="{{ route('documents.delete.pelatih') }}" method="POST" onsubmit="return confirm('Hapus dokumen ini?')">
                                                @csrf
    ...
    <script>
        // Form submission code (dihapus/dinonaktifkan karena menggunakan form submission normal)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set default tanggal gabung to today (optional, tapi saya biarkan)
        ...
        // (Removed) periodic notification animation to avoid null reference errors on pages without the elements
    ...
    <script>
        // Force-submit the Pelatih form when Simpan Data is clicked (fallback if default submit is blocked)
        document.addEventListener('DOMContentLoaded', function(){
            var saveBtn = document.getElementById('submitBtn');
            var form = document.getElementById('pelatihForm');
            if(saveBtn && form){
    ...
    <form id="uploadPelatihForm" action="{{ route('documents.upload.pelatih') }}" method="POST" enctype="multipart/form-data">
        @csrf
    </form>
</body>
</html>