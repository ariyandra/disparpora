...
<head>
    ...
    <title>Tambah Jadwal - Dinas Pariwisata Pemuda dan Olahraga</title>
...
<body>
        ...
        <div class="page-container">
            ...
            <div class="page-header-card">
                <div class="flex justify-between items-center">
                    ...
                    <a href="{{ route('jadwal') }}" class="btn-tambah" style="background: linear-gradient(135deg, #6b7280, #4b5563); text-decoration: none;">
                        <span>←</span> Kembali
                    </a>
                </div>
            </div>
            ...
            <div class="table-container-card">
                <div class="table-header" style="border-bottom: none; margin-bottom: 0; padding-bottom: 0;">
                    <h2 class="table-title">Form Data Jadwal</h2>
                </div>
                
                <form id="jadwalForm" action="{{ route('simpan.ubah.jadwal') }}" method="POST" style="margin-top: 30px;">
                    @csrf
                    {{-- Validation messages --}}
                    @if ($errors->any())
                        <div style="margin-bottom:16px; padding:12px; border-radius:8px; background:#fff5f5; color:#b91c1c;">
                            <strong>Ada beberapa kesalahan input:</strong>
                    ...
                    @endif
                    @if(session('success'))
                        <div style="margin-bottom:16px; padding:12px; border-radius:8px; background:#ecfccb; color:#365314;">
                            {{ session('success') }}
                        </div>
                    @endif
                    @if(session('error'))
                        <div style="margin-bottom:16px; padding:12px; border-radius:8px; background:#fff7ed; color:#7c2d12;">
                            {{ session('error') }}
                        </div>
                    @endif
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 25px; position: relative;">
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            ...
                            <label for="cabang_olahraga" style="font-size: 14px; font-weight: 600; color: #333; margin-bottom: 8px;">
                            ...
                            <select id="cabang_olahraga" 
                                ...
                                @foreach ($dataCabor as $cabor)
                                    <option value="{{ $cabor->id }}" {{ (old('cabang_olahraga', $jadwal->id_cabor) == $cabor->id) ? 'selected' : '' }}>{{ $cabor->nama_cabor }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            <div style="position: absolute; top: 0; left: 0; width: 4px; height: 100%; background: border-radius: 2px; opacity: 0; transition: opacity 0.3s ease;" class="field-indicator"></div>
                            <label for="lapangan" style="font-size: 14px; font-weight: 600; color: #333; margin-bottom: 8px;">
                                🏟️ Lapangan <span style="color: #ff6b6b;">*</span>
                            </label>
                            <select id="lapangan" 
                                    name="lapangan" 
                                    required
                                ...
                                @foreach ($dataLapangan as $lapangan)
                                    <option value="{{ $lapangan->id }}" {{ (old('lapangan', $jadwal->id_lapangan) == $lapangan->id) ? 'selected' : '' }}>{{ $lapangan->nama_lapangan }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            <div style="position: absolute; top: 0; left: 0; width: 4px; height: 100%; background: border-radius: 2px; opacity: 0; transition: opacity 0.3s ease;" class="field-indicator"></div>
                            <label for="jam_mulai" style="font-size: 14px; font-weight: 600; color: #333; margin-bottom: 8px;">
                                🕐 Jam Mulai <span style="color: #ff6b6b;">*</span>
                            </label>
                <input type="time" 
                    id="jam_mulai" 
                    name="jam_mulai"
                    required
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden;">
                            <div style="position: absolute; top: 0; left: 0; width: 4px; height: 100%; background: border-radius: 2px; opacity: 0; transition: opacity 0.3s ease;" class="field-indicator"></div>
                            <label for="jam_selesai" style="font-size: 14px; font-weight: 600; color: #333; margin-bottom: 8px;">
                                🕕 Jam Selesai <span style="color: #ff6b6b;">*</span>
                            </label>
                <input type="time" 
                    id="jam_selesai" 
                    name="jam_selesai" 
                    required
                        ...
                        <div style="display: flex; flex-direction: column; position: relative; overflow: hidden; grid-column: 1 / -1;">
                            ...
                            <textarea id="keterangan" 
                                      ...
                                      placeholder="Masukkan keterangan tambahan untuk jadwal ini (opsional). Contoh: Latihan rutin mingguan, persiapan turnamen, dll..."
                                      onfocus="this.style.borderColor='#667eea'; this.style.boxShadow='0 0 0 3px rgba(102, 126, 234, 0.1)'"
                                      onblur="this.style.borderColor='#e0e0e0'; this.style.boxShadow='none'">{{ $jadwal->keterangan }}</textarea>
                        </div>
                    </div>
    ...
    <script>
        ...
        const jadwalFormEl = document.getElementById('jadwalForm');
        if (jadwalFormEl) {
            jadwalFormEl.addEventListener('submit', function(e) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set default tanggal to today only if field is empty
            ...
            // Note: dynamic replacement of lapangan options was removed to keep
            // option values as IDs provided by the server. This avoids validation
            // failures where the controller expects an existing lapangan id.
        });
