...
<head>
    ...
    <title>Pelatih - Dinas Pariwisata Pemuda dan Olahraga</title>
...
<body>
    <nav class="navbar">
        <div class="nav-container">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item">
                    <a href="{{ route('cabor') }}" class="nav-link">Cabang Olahraga</a>
                </li>
                <li class="nav-item">
    ...
    <main class="main-content">
        <div class="page-container mt-32 sm:mt-40">
            ...
            <div id="importBox" style="display:none;padding:12px;border:1px dashed #e5e7eb;margin-bottom:16px;border-radius:8px;">
                ...
                @endif
                <form id="importForm" action="{{ route('import.pelatih.submit') }}" method="POST" enctype="multipart/form-data">
                    @csrf
            ...
            <div class="table-container-card">
                <div class="table-header">
                    <h2 class="table-title">Daftar Pelatih</h2>
                    <div class="search-container">
                        <span class="search-icon">🔍</span>
                        <input type="text" class="search-input" placeholder="Cari pelatih..." id="searchInput">
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                @if(!empty($pelatih->foto))
                                    <img src="{{ asset('storage/' . $pelatih->foto) }}" alt="Foto {{ $pelatih->nama }}" style="width:60px;height:60px;object-fit:cover;border-radius:6px;cursor:pointer;">
                                @else
                            ...
                            <td>{{ $pelatih->cabor->nama_cabor }}</td>
                            <td>{{ $pelatih->tanggal_lahir }}</td>
                            <td>{{ $pelatih->tanggal_gabung }}</td>
                            <td>
                                @php($docs = $pelatih->documents ?? collect())
                                @if($docs->count())
                                    <details>
                            ...
                            <td>
                                <div class="action-buttons" style="gap:8px; display:flex; flex-direction:column; align-items:flex-start;">
                                    ...
                                    @if($role == 2 && ($pelatih->status_verifikasi ?? '') === 'pending_kecamatan')
                                        <form action="{{ route('verifikasi.approve') }}" method="POST" style="display:inline-flex;gap:6px;">
                                            @csrf
                                            <input type="hidden" name="entity" value="pelatihs">
                                    ...
                                    @endif
                                    @if(in_array($role,[0,1]) && ($pelatih->status_verifikasi ?? '') === 'pending_admin')
                                        <form action="{{ route('verifikasi.approve') }}" method="POST" style="display:inline-flex;gap:6px;">
                                            @csrf
                                            <input type="hidden" name="entity" value="pelatihs">
                                            <input type="hidden" name="id" value="{{ $pelatih->id }}">
                                    ...
                                    @endif
                                    <form action="{{ route('update.pelatih') }}" method="GET">
                                        <input type="hidden" value="{{ $pelatih->id }}" name="id_pelatih">
                                        <button type="submit" class="btn-action btn-edit">
                                            <span>✏️</span> Edit
                                        </button>
                                    </form>

                                    <form action="{{ route('hapus.pelatih') }}" method="POST">
                                        @csrf
                                        <input type="hidden" value="{{ $pelatih->id }}" name="id_pelatih">
                ...
                <!-- Pagination (prev/next kept; numeric page buttons are generated by JS) -->
                <div class="pagination">
                    <button class="pagination-btn" onclick="changePage('prev')">‹ Sebelumnya</button>
                    <!-- JS will insert page-number buttons here -->
    ...
    <script>
        ...
        window.addEventListener('load', function() {
            const tableRows = document.querySelectorAll('#pelatihTableBody tr');
        ...
        (function(){
            ...
            document.addEventListener('click', function(e){
                ...
                if(target.tagName === 'IMG' && target.closest('#pelatihTableBody')){
                    // open modal
